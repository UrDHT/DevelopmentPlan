...
For every DHT we tried, our greedy approximation of Delaunay Triangulation %using a distance and midpoint function 
...
\item Similarly to \texttt{distance}, \texttt{midpoint} takes in the points $ a $ and $ b $ and returns the closest midpoint\footnote{Toroidal spaces have more than one midpoint.} between the two.
...
The largest complication in implementing Kademlia is the exclusive or, or XOR, metric which is used for distance.
...
\[ disance(a, b) = midpoint(a, b) = a  \oplus b\]

We then implemented handle long peers.

\subsection{Implementing A DHT in an Arbitrary Euclidean Geometry}

We used a toroidal Euclidean geometry as the default space when building UrDHT and DGVH \cite{dgvh}.
This uses the following \texttt{distance} and \texttt{midpoint} functions.
...
\[midpoint\left(\vec{a}, \vec{b}\right) =  \left[\frac{(a_0, b_0)}{2},\frac{(a_1, b_1)}{2}, ... ,\frac{(a_d, b_d)}{2} \right] \]
