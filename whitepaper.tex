...
By completing a few simple functions, a developer can implement the topology of any DHT in any arbitrary space.

%Latency embedding
One topology of particular interest we created is a DHT operating within a Poincar\'{e} disk model.
This DHT has latency embedded within the overlay and capable of responding to changes in latency.
...
We found that we could reproduce the topology of different DHTs by defining the necessary distance and midpoint functions for the space.  %TODO and what else

The end result is not only do we have an abstracted DHT, we have a simple framework that developers can use to quickly create new distributed applications and frameworks.


\subsubsection{Bootstrapping}
Another ambiguous issue with DHTs and DHT-based P2P applications we wish to address with UrDHT is the what we have termed the \textit{bootstrapping problem}.
...
The general way this works is by having a potential user manually look up the bootstrapping information needed at a centralized source, such as the project or application's website.
There is a philosophical conflict here:  a distributed application using a centralized source of information to build a distributed network.
...
This is obviously undesirable, but is the status quo in DHTs.
...
\begin{itemize}
	...
	We list a set  of simple functions that, once defined, allow our Distributed Greedy Voronoi Heuristic to be run in any space, creating a DHT overlay for that space (Section \ref{sec:define}).
	...
	We show how to reproduce the topology of Chord and Kademlia using UrDHT, which we call UrChord and UrKademlia (Section \ref{sec:urdht}).
