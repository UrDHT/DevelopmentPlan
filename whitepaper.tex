...
The DHT Protocol (\texttt{LogicClass.py}) is the shared functionality between every single DHT.
...
Each maintenance cycle, the node recalculates its Delaunay (short) peers using its neighbor's peer list and any nodes that have notified it since the last maintance cycle.
...
Euclidean space was the default space we used when building UrDHT.
...
Creating a DHT overlay that operates in a hyperbolic geometry
